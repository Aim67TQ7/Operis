...
### Browser staging receipt, 2026-09-08

- Browser-flow application commit: `ff5281bdd890c33aa6d742d3cb6402fed1072be6`. Final API body-bound hardening: `483d7bb20d15dded5f346366ffc55d920b6044b7`, passed [GitHub Actions run 34210719497](https://github.com/Aim67TQ7/Operis/actions/runs/34210719497). Source and lockfiles match the local checks above.
- Pete's existing API container is Healthy at `operis-api:483d7bb20d15dded5f346366ffc55d920b6044b7`, image `sha256:29fc86f0c2e0a86c58224e6b492c68344c54303d3364aac06826c7b8f42cd2e7`. The existing mode-600 environment and ingestion capability were preserved. Only the authorized test destination/company mapping and release tag were added/updated. No database or Auth mutations were performed for this continuation.
- Netlify staging deploy `6a9fd60b15d51b7c8833a722` publishes the browser frontend from `ff5281b`; the later commit changes only API/tests/evidence. Public JavaScript and CSS bytes match the built artifacts. JavaScript SHA-256: `576611513b0ebe7ea48d9633f73b65dbbd0ccf61f03c4b8e76f91389dc568340`; CSS SHA-256: `fdad538863513492c09bc0cb64199ad494be8cd9c99b68dc026435de2125af01`.
- The normal signed-in Chrome session renders the new browser-first instructions, optional local kit disclosure, setup button and assessment history. An existing unconfigured company returns the explicit setup error before any credential form appears. Sanitized backend logs independently confirm HTTP 409, request `b015b448-1937-434e-ad69-7f3131bf1669`, duration 141 ms.
- Public proxy checks: anonymous setup GET 401 (`9e103d7f-e16a-4a96-a99e-3a6bd53e8edb`); scan POST without Origin 403 (`791d370e-8b2d-407d-9d73-cd153004cab2`); anonymous scan POST 401 (`b17e89b1-d120-44e3-af01-4fddc8cdd410`); anonymous save POST 401 (`fe647b38-dbb4-465a-ba75-b8ee69bb67b3`). All returned no-store and correlation IDs. Final deployed API readiness returned 200/no-store, request `2aae6276-0634-4de2-b5e6-63306719ee80`.
- The open browser account belongs to a different organization from the existing customer administrator. Read-only operator inspection confirmed the customer administrator and zero registered companies; membership, company setup and credentials were not fabricated. The user must sign into that provisioned organization, add the intended company through the normal authorized UI, and supply the existing Epicor account/API key in the new form. A real browser scan, saved assessment/audit read-back and pilot click remain **not yet verified**. PR #3 remains draft.

Rollback: restore the earlier API image/release and frontend deploy recorded in the September 7 receipt. Keep the existing discovery schema, capability and audit history. The browser destination map can be set to `{}` to disable new scans without changing identity or data permissions.
