...
Source branch: `build/phase-1-foundation` in Aim67TQ7/Operis. Main remains a README shell. Existing source has a JavaScript-rendered organization workspace with generic workspace metadata, no resource pages, and no sitemap or robots file. Those are source findings, not a completed audit of Google's index.

The staging deployment inspected for this PR had no recorded commit ref or repository binding. Source-to-deploy parity is unverified. The public web fetch did not return usable site HTML. No Search Console, Analytics, Ads, or keyword-volume account was inspected. No production domain is established in the inspected configuration.
...
- Staging canonical behavior: use `https://operis-staging.netlify.app` for staging builds only, with indexing disabled.
- First conversion path: checklist CTA only. No lead capture form, CRM event, payment path, or public scanner download is part of this release.
- SyteLine target language: prioritize both Infor SyteLine and CloudSuite Industrial.

## Remaining launch work

1. Confirm the authoritative public marketing domain before enabling indexing and reconcile this branch with the uploaded website. Preserve any newer discovery/marketing work before merging. Do not replace a separately maintained marketing site with the older workspace source.
...
3. Review the two articles and validate product claims against the latest build before publishing them.
4. Keep the first CTA pointed at the readiness checklist until a real lead conversion destination is approved. The current CTA reaches an actual readiness checklist; it does not collect a lead.
